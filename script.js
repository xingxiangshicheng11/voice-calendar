...
function parseVoiceCommand(text) {
  ...
  if (text.includes('删除')) {
    ...
    if (dateMatch) {
      ...
      if (toDelete.length > 0) {
        ...
        speak(`已删除${targetDate}的事件`);
      } else {
        speak(`${targetDate}没有找到事件`);
      }
      return;
    }
    // 按关键词删除
    if (text.includes('今天')) {
      const todayStr = getTodayStr();
      const toDelete = events.filter(e => e.dateStr === todayStr);
      if (toDelete.length > 0) {
        toDelete.forEach(e => deleteEvent(e.id));
        speak(`已删除今天的所有事件`);
      } else {
  ...
  // ========== 3. 查询事件 ==========
  if (text.includes('今天有什么') || text.includes('今天的事件') || text === '今天') {
    const todayStr = getTodayStr();
  ...
  if (text.includes('明天有什么') || text.includes('明天的事件') || text === '明天') {
    const tomorrow = new Date();
    tomorrow.setDate(tomorrow.getDate() + 1);
  ...
  if (text.includes('明天')) {
    ...
    // 提取标题：去掉"明天"、"下午"、"上午"、"点"、"分"等词
    title = text.replace(/明天|今天|下午|上午|点|分|半/g, '').trim();
    if (title === '' || title === '明天') {
      title = "提醒事项";
    }
  }
  // 4.2 处理 "后天"
    ...
    title = text.replace(/后天|下午|上午|点|分|半/g, '').trim();
    if (title === '' || title === '后天') {
      title = "提醒事项";
    }
  }
  ...
  if (dateStr && title) {
    // 检查是否重复（可选）
    const exists = events.some(e => e.dateStr === dateStr && e.title === title);
    if (exists) {
      speak(`${dateStr}已有相同事件，不重复添加`);
      showFeedback(`⚠️ ${dateStr}已有相同事件`);
    } else {
